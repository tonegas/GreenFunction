...
int main(int argc, const char *argv[])
{
  ...
  const int freq = 50;
  while (running && fread(
                        scenario_msg.data_buffer,
                        sizeof(scenario_msg.data_buffer), 1, fp_scenario
                    ) > 0)
  {
    auto start = std::chrono::high_resolution_clock::now();
    std::cout << "cycle_number:" << scenario_msg.data_struct.CycleNumber
              << std::endl;
    if (scenario_msg.data_struct.CycleNumber == 1 ||
        scenario_msg.data_struct.CycleNumber % 50 == 0)
    {
      if (scenario_msg.data_struct.CycleNumber > 1)
      {
        /* RICEZIONE VIA ZMQ */
        ...
        while (received == 0)
        {
          ...
          auto data =
              flatbuffers::GetRoot<IGreen>(zmq_msg_data(&received_data));

          cycle_number = data->cycle_number();
          std::cout << "received <- cycle_number: " << cycle_number
                    << std::endl;
          ecu_up_time = data->out()->ecu_up_time();
          t0          = data->out()->t0();
          auto velocity_profile_times_aux =
              data->out()->velocity_profile_times();
          auto velocity_profile_values_aux =
              data->out()->velocity_profile_values();
          velocity_profile_times = std::vector<double>(
              velocity_profile_times_aux->begin(),
              velocity_profile_times_aux->end()
          );
          velocity_profile_values = std::vector<double>(
              velocity_profile_values_aux->begin(),
      ...
      auto packet = CreateIGreen(
          builder, scenario_msg.data_struct.CycleNumber, &green_struct
      );
      ...
      std::cout << "sent -> cycle_number:"
                << scenario_msg.data_struct.CycleNumber << std::endl;
    }
    // Compute codriver
